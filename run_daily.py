...
for pk, rec in omap.items():
    ...
    # v8.0: the placeholder test now reads the RAW prices, not the rounded
    # no-vig. odds.py stores novig at 4dp, so any equality test on it inherits
    # a 5e-5 tolerance nobody chose -- that rounding is exactly how the v7.4
    # exemption misfired on Texas (nine-book consensus 0.5000051 stored as
    # 0.5). "books_used <= 1 and homeML == awayML" describes an actual
    # lone-book placeholder and cannot be tripped by averaging or rounding.
    placeholder = ((rec.get('books_used') or 1) <= 1
                   and rec.get('homeML') is not None
                   and rec.get('homeML') == rec.get('awayML'))
